...
func (cfg *config) applier(i int, applyCh chan ApplyMsg) {
	// cfg.t.Logf("reads message from apply ch")
	for m := range applyCh {
	...
	for m := range applyCh {
		if m.SnapshotValid {
			...
			if cfg.rafts[i].CondInstallSnapshot(m.SnapshotTerm,
				...
				// 为每个peer维护一个map：{SnapshotIndex, v(snapshot)}
				...
				// 这里就相当于规定apply snapshot过程之中不能插入apply其它log
				lastApplied = m.SnapshotIndex
				DPrintf("InstallSnapshot Successfully")
			}
			cfg.mu.Unlock()
			...
			if (m.CommandIndex+1)%SnapShotInterval == 0 {
				DPrintf("apply %v call Snapshot", m.CommandIndex)
				w := new(bytes.Buffer)
...
func (cfg *config) nCommitted(index int) (int, interface{}) {
	...
	for i := 0; i < len(cfg.rafts); i++ {
		...
		if ok {
			if count > 0 && cmd != cmd1 {
				cfg.t.Fatalf("committed values do not match: index %v, %v(server %v), %v(server %v)\n",
					index, cmd, i-1, cmd1, i)
			}
			DPrintf("[%v] committed log where index is %v", i, index)
			count += 1
			cmd = cmd1
		}
	}
...
// 会循环测试10s，测试所有的server，可能在开始时是错的leader，就会提交失败
// 然后尝试一个新的leader re-submit。
// retry为true，可能会多次提交command，以防leader在start()后失败
// retry为false，会调用start()一次，以简化2B测试
func (cfg *config) one(cmd interface{}, expectedServers int, retry bool) int {
	...
	DPrintf("commited count: %d", count)
	DPrintf("index: %d", index2)
	cfg.t.Fatalf("one(%v) failed to reach agreement", cmd)
	return -1
}
