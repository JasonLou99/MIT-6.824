...
func (rf *Raft) CondInstallSnapshot(lastIncludedTerm int, lastIncludedIndex int, snapshot []byte) bool {
	...
	// }
	return lastIncludedIndex > rf.commitIndex
}
...
func (rf *Raft) Snapshot(index int, snapshot []byte) {
	...
	w := new(bytes.Buffer)
	e := labgob.NewEncoder(w)
	e.Encode(rf.getLastLogIndex())
	e.Encode(rf.getLastLogTerm())
	w.Bytes()
	...
	DPrintf("after trim log, rf.log: %v", rf.log)
...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if args.PrevLogIndex >= rf.log[0].Index && rf.log[tagPrevLogIndex].Term != args.PrevLogTerm {
		// 日志不一致，需要将PrevLog向前移动比较，这里直接越过了与PrevLog同term的一批日志
		term := rf.log[tagPrevLogIndex].Term
		...
		rf.log = append(rf.log[:tagPrevLogIndex+1], args.Entries...)
		reply.Success = true
	...
	for server := range rf.peers {
		if server != rf.me && rf.state == Leader {
			if rf.nextIndex[server] > rf.log[0].Index {
				...
				DPrintf("[%d](term %d, state %d) sendInstallSnapshot to [%d]", server, rf.currentTerm, rf.state, server)
...
func (rf *Raft) applyToService(applyCh chan ApplyMsg) {
	...
	for range rf.chanApplyLog {
		...
		DPrintf("[%d](term %d, state %d) apply command lenth is %d", rf.me, rf.currentTerm, rf.state, len(applyLogs))
