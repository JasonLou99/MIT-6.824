...
// committed log entry.
// log条目成功提交时，每个peer通过传递applyCh到Make()来发送ApplyMsg到同一个服务器上的服务中
...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	if args.Entries == nil {
		...
		if rf.currentTerm > args.Term {
			...
			// 认同leader
			// 更新心跳时间，更新term，让candidate变会follower
			// rf.mu.Lock()
			// defer rf.mu.Unlock()
...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	// 目标服务器调用该方法，处理投票请求
	// 先看votedFor
	// 这里需要注意 rf.votedFor == args.CandidateId 的情况，
	// 但我觉得应该不会在相同term内重复发送投票请求给同peer两次，
	// 但论文写了这种情况，就先不处理 看看具体问题情况再处理
	rf.mu.Lock()
	defer rf.mu.Unlock()
	if rf.votedFor == -1 || rf.votedFor == args.CandidateId {
		// 先比较term
		if rf.currentTerm < args.Term {
			// 但是 candidate日志可能更旧 吗？
		...
		if rf.currentTerm == args.Term {
			...
			if rf.log[rf.lastApplied].Term == args.LastLogTerm {
				if rf.commitIndex <= args.LastLogIndex {
					...
					// 自己的log index 更大，说明log更新
					reply.Term = args.Term
					reply.VoteGranted = false
					return
				}
			} else {
				// 自己的log term更大，说明log更新
				reply.Term = args.Term
				reply.VoteGranted = false
				return
			}
		...
		// 可能是以前投的
		if rf.currentTerm < args.Term {
			DPrintf("%d(term(old) %d) has voted in old term", rf.me, rf.currentTerm)
			// rf.mu.Lock()
			reply.Term = args.Term
			reply.VoteGranted = true
			rf.votedFor = args.CandidateId
...
func (rf *Raft) sendRequestVote(server int, args *RequestVoteArgs, reply *RequestVoteReply) bool {
	DPrintf("%d(term %d) sends RequestVote RPC to %d", rf.me, rf.currentTerm, server)
...
func (rf *Raft) election() {
	...
	finished := 0
	// fail := false
	// 转变身份，term+1，广播给所有peer（包括自己）投票rpc
	rf.becomeCandidate()
	// 这个外层不能套锁，因为收到新leader心跳后需要获取锁修改state
	for rf.state == Candidate {
		...
		if count >= (len(rf.peers)/2)+1 {
			...
			for index, _ := range rf.peers {
				go func(i int) {
					...
					DPrintf("%d(term %d) send heartbeat to %d", rf.me, rf.currentTerm, i)
		...
		if finished == len(rf.peers) {
			...
			// 未能收到半数以上投票，并且自身还是候选者
			// 随机等待一段时间再开始选举
			// 从[0,10)随机选取
			// 100~200ms重新开始选举，因为leader会等待80ms才发送心跳
			randTime := rand.Intn(10)*10 + 100
...
func (rf *Raft) becomeCandidate() {
	// follower --> candidate
	DPrintf("%d(term %d) becomes Candidate", rf.me, rf.currentTerm)
	rf.mu.Lock()
...
func (rf *Raft) becomeLeader() {
	DPrintf("%d(term %d) becomes Leader", rf.me, rf.currentTerm)
	// DPrintf("%d(term %d) rf.state %d", rf.me, rf.currentTerm, rf.state)
	rf.mu.Lock()
	if rf.state == Candidate {
		rf.state = Leader
		rf.isLeader = true
		DPrintf("%d(term %d) isLeader is true", rf.me, rf.currentTerm)
	}
	rf.mu.Unlock()
}
...
func (rf *Raft) ticker() {
	for !rf.killed() {
		...
		if time.Since(rf.lastHeartBeat) > time.Duration(rf.electionTimeOut)*time.Millisecond {
			// 超时需要开始选举
			DPrintf("%d(term %d) elect timeout", rf.me, rf.currentTerm)
			rf.election()
		}
	}
}
...
func (rf *Raft) sendHeartBeat() {
	for !rf.killed() {
		...
		if rf.isLeader {
			...
			mu2.Lock()
			if finished_hb != len(rf.peers) {
				cond2.Wait()
			} else {
				// 所有心跳返回结果都为true，说明leader被认可
				if count_hb != len(rf.peers) {
					// leader不被认可
					rf.becomeFollower()
					break
				}
...
func Make(peers []*labrpc.ClientEnd, me int,
	...
	rf.log = append(rf.log, Log{"initial", 0})
