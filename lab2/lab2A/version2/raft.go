...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	if (rf.votedFor == -1 || rf.votedFor == args.CandidateId) && rf.isUpToDate(args.LastLogTerm, args.LastLogIndex) {
		DPrintf("%d(term %d) vote to %d", rf.me, rf.currentTerm, args.CandidateId)
...
func (rf *Raft) sendRequestVote(server int, args *RequestVoteArgs, reply *RequestVoteReply) bool {
	...
	if ok {
		...
		if reply.VoteGranted {
			rf.voteCount++
			DPrintf("%d(term %d) voteCount is %d", rf.me, rf.currentTerm, rf.voteCount)
...
func (rf *Raft) sendHeartBeat() {
	...
	for server := range rf.peers {
		if server != rf.me && rf.state == Leader {
			DPrintf("%d(term %d) send heartbeat to %d", rf.me, rf.currentTerm, server)
...
func (rf *Raft) Run() {
	for {
		...
		case Follower:
			select {
			// 进行投票，无事发生（认可candidate，则自身状态还是follower）
			case <-rf.chanGrantVote:
			// 收到正常心跳，就会写入管道触发，无事发生
		...
		case Candidate:
			...
			// 平分选票，则上面管道不会触发，会随机等待时间（300~500ms）循环选举
			case <-time.After(time.Millisecond * time.Duration(rand.Intn(10)*20+300)):
				DPrintf("集体竞选失败，重新选举")
			}
		case Leader:
			// 只需要发送心跳
