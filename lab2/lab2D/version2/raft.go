...
func (rf *Raft) persist() {
	// 持久化数据（伪持久化，只是写入了persister.raftstate）
	DPrintf("[%d](term %d, state %d) persist log's length: %d", rf.me, rf.currentTerm, rf.state, len(rf.log))
...
func (rf *Raft) readPersist(data []byte) {
	...
	if d.Decode(&currentTerm) != nil || d.Decode(&votedFor) != nil || d.Decode(&log) != nil {
		...
		rf.log = log
		DPrintf("[%d](term %d, state %d) read persisted log's length: %d", rf.me, rf.currentTerm, rf.state, len(rf.log))
		rf.mu.Unlock()
	}
...
func (rf *Raft) trimLog(lastIncludedIndex int, lastIncludedTerm int) {
	// 因为实际记录client的日志应该是从1开始
	// 所以这里0处的位置只是占位置，类似之前的initial log
	logs := make([]Log, 0)
	log := Log{
...
//
// 上层服务想要切换到快照。
// 只有在Raft没有更多最新信息的情况下才这么做，因为它在applyCh上传达了快照。
func (rf *Raft) CondInstallSnapshot(lastIncludedTerm int, lastIncludedIndex int, snapshot []byte) bool {
	// 发送快照和应用快照应该是原子操作，不能有新的日志commit，导致快照过时
	// Your code here (2D).
	// if rf.lastApplied == lastIncludedIndex {
	// 	rf.trimLog(lastIncludedIndex, lastIncludedTerm)
	// }
	if lastIncludedIndex < rf.lastApplied {
		DPrintf("[%d](term %d, state %d) reject snapshot, lastIncludedIndex is %v, rf.lastApplied is %v", rf.me, rf.currentTerm, rf.state, lastIncludedIndex, rf.lastApplied)
		// 复原index和term
		...
		// 满足apply快照条件
		DPrintf("[%d](term %d, state %d) accept snapshot, lastIncludedIndex is %v, rf.lastApplied is %v", rf.me, rf.currentTerm, rf.state, lastIncludedIndex, rf.lastApplied)
		rf.trimLog(lastIncludedIndex, lastIncludedTerm)
		DPrintf("[%d](term %d, state %d) trim log, rf.log: %v", rf.me, rf.currentTerm, rf.state, rf.log)
...
// 服务说它已经创建了一个快照，包含所有的信息，包括index。
// 这意味着服务不再需要通过(并包含)该索引的日志。raft现在应该尽可能地修剪它的log。
// 必须使用SaveStateAndSnapshot()将每个快照存储在持久对象中。
// service ----> raft
func (rf *Raft) Snapshot(index int, snapshot []byte) {
	...
	rf.mu.Lock()
	defer rf.mu.Unlock()
	DPrintf("in Snapshot() arg index: %d", index)
	rf.lastIncludedIndex = rf.log[0].Index
...
func (rf *Raft) InstallSnapshot(args *InstallSnapshotArgs, reply *InstallSnapshotReply) {
	...
	if args.Term > rf.currentTerm {
		// 更新term，根据日志情况考虑
		DPrintf("[%d](term %d, state %d) call persist() in InstallSnapshot() where args.Term(%d) > rf.currentTerm(%d)", rf.me, rf.currentTerm, rf.state, args.Term, rf.currentTerm)
		rf.currentTerm = args.Term
		rf.state = Follower
		rf.votedFor = -1
		rf.persist()
	}
	// 必须设置term，否则leader会变成term为0的Follower导致[-1]报错
	reply.Term = args.Term
	rf.chanHeartbeat <- true
	// Follower's logs are outdated
...
func (rf *Raft) sendInstallSnapshot(server int, args *InstallSnapshotArgs, reply *InstallSnapshotReply) {
	...
	if reply.Term > rf.currentTerm {
		...
		rf.state = Follower
		rf.votedFor = -1
		DPrintf("[%d](term %d, state %d) call persist() in sendInstallSnapshot()", rf.me, rf.currentTerm, rf.state)
		rf.persist()
		return
	}
	// 对该Follower的日志进行更新
...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if args.Term > rf.currentTerm {
		// 更新term，根据日志情况考虑
		DPrintf("[%d](term %d, state %d) call persist() in AppendEntries() where args.Term(%d) > rf.currentTerm(%d)", rf.me, rf.currentTerm, rf.state, args.Term, rf.currentTerm)
		rf.currentTerm = args.Term
		rf.state = Follower
		rf.votedFor = -1
		rf.persist()
	...
	if args.PrevLogIndex < rf.log[0].Index {
		// leader's nextIndex need to update
		reply.RetryIndex = rf.getLastLogIndex() + 1
		reply.Success = false
		return
	}
	if args.PrevLogIndex >= rf.log[0].Index && rf.log[args.PrevLogIndex-rf.log[0].Index].Term != args.PrevLogTerm {
		...
		// }
		// 追加日志
		rf.log = append(rf.log[:args.PrevLogIndex-rf.log[0].Index+1], args.Entries...)
		if len(args.Entries) > 0 {
			DPrintf("[%d](term %d, state %d) append logs from %d to %d", rf.me, rf.currentTerm, rf.state, args.PrevLogIndex+1, args.PrevLogIndex+len(args.Entries))
			DPrintf("[%d](term %d, state %d) logs: %v", rf.me, rf.currentTerm, rf.state, rf.log)
		}
		reply.Success = true
	...
	DPrintf("[%d](term %d, state %d) call persist() in AppendEntries()", rf.me, rf.currentTerm, rf.state)
	rf.persist()
}
...
func (rf *Raft) sendAppendEntries(server int, args *AppendEntriesArgs, reply *AppendEntriesReply) bool {
	...
	if reply.Term > rf.currentTerm {
		DPrintf("[%d](term %d, state %d) call persist() in AppendEntries() where reply.Term(%d) > rf.currentTerm(%d)", rf.me, rf.currentTerm, rf.state, reply.Term, rf.currentTerm)
		rf.currentTerm = reply.Term
		rf.state = Follower
		rf.votedFor = -1
		rf.persist()
	...
	if !reply.Success {
		...
		if len(args.Entries) > 0 {
			DPrintf("[%d](term %d, state %d) update matchIndex[%d] and nextIndex[%d] to %d, %d", rf.me, rf.currentTerm, rf.state, args.Entries[len(args.Entries)-1].Index, server, server, args.Entries[len(args.Entries)-1].Index+1)
			rf.matchIndex[server] = args.Entries[len(args.Entries)-1].Index
			rf.nextIndex[server] = rf.matchIndex[server] + 1
		}
	}
	// 返回失败，也有可能需要leader apply日志
	for i := rf.getLastLogIndex(); i > rf.commitIndex && rf.log[i-rf.log[0].Index].Term == rf.currentTerm; i-- {
		...
		if count > len(rf.peers)/2 {
			rf.commitIndex = i
			DPrintf("[%d](term %d, state %d) commitIndex is %d", rf.me, rf.currentTerm, rf.state, rf.commitIndex)
			rf.chanApplyLog <- true
			break
		}
	}
...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	// args.term更大不一定会投票，但是自己一定会变为Follower
	if args.Term > rf.currentTerm {
		// become follower and update current term
		DPrintf("[%d](term %d, state %d) call persist() in RequestVote() where args.Term(%d) > rf.currentTerm(%d)", rf.me, rf.currentTerm, rf.state, args.Term, rf.currentTerm)
		rf.state = Follower
		rf.currentTerm = args.Term
		rf.votedFor = -1
		rf.persist()
	...
	// args.term >= rf.term && log is up-to-date , vote successfully
	reply.Term = rf.currentTerm
	reply.VoteGranted = false
	...
	if (rf.votedFor == -1 || rf.votedFor == args.CandidateId) && rf.isUpToDate(args.LastLogTerm, args.LastLogIndex) {
		...
		DPrintf("[%d](term %d, state %d) call persist() in RequestVote() to update votedFor", rf.me, rf.currentTerm, rf.state)
		rf.persist()
...
// 	if rf.snapshot == nil || len(rf.snapshot) < 1 { // bootstrap without any state?
// 		return -1
// 	}
...
func (rf *Raft) Start(command interface{}) (int, int, bool) {
	...
	if rf.state != Leader {
		...
		DPrintf("[%d](term %d, state %d) call persist() in Start()", rf.me, rf.currentTerm, rf.state)
		rf.persist()
	...
	for server := range rf.peers {
		if server != rf.me && rf.state == Leader {
			if rf.nextIndex[server] > rf.log[0].Index {
				...
				DPrintf("[%d](term %d, state %d)'s nextIndex[%d] is %d", rf.me, rf.currentTerm, rf.state, server, rf.nextIndex[server])
				if rf.nextIndex[server] <= rf.getLastLogIndex() {
					DPrintf("[%d](term %d, state %d) send logs(%v) to [%d]", rf.me, rf.currentTerm, rf.state, args.Entries, server)
					args.Entries = rf.log[tagIndex+1:]
				}
				args.LeaderCommit = rf.commitIndex
				go rf.sendAppendEntries(server, args, &AppendEntriesReply{})
			} else {
				// 该server的日志落后与leader第一条日志，无法靠appendentries恢复，需要通过InstallSnapshot方式恢复状态
				DPrintf("[%d](term %d, state %d) sendInstallSnapshot to [%d]", rf.me, rf.currentTerm, rf.state, server)
...
func (rf *Raft) Run() {
	...
	for !rf.killed() {
		...
		case Candidate:
			...
			rf.votedFor = rf.me
			DPrintf("[%d](term %d, state %d) call persist() in run() to update votedFor", rf.me, rf.currentTerm, rf.state)
			rf.persist()
			rf.voteCount = 1
			rf.mu.Unlock()
			go rf.election()
			...
			case <-time.After(time.Millisecond * time.Duration(rand.Intn(300)+200)):
				DPrintf("[%d](term %d, state %d) restart election", rf.me, rf.currentTerm, rf.state)
			}
		}
	}
}
...
func Make(peers []*labrpc.ClientEnd, me int,
	...
	rf.chanApplySnapshot = make(chan bool, 100)
	go rf.Run()
	go rf.applyToService(applyCh)
	go rf.applySnapshot(applyCh)
...
func (rf *Raft) applySnapshot(applyCh chan ApplyMsg) {
	for range rf.chanApplySnapshot {
		applyMsg := ApplyMsg{
			SnapshotValid: true,
		...
		DPrintf("[%d](term %d, state %d) apply snapshot", rf.me, rf.currentTerm, rf.state)
		// rf.lastApplied = rf.lastIncludedIndex
		applyCh <- applyMsg
	}
}
